    ...
    methods
        ...
        function this = extractDbaseFeats(this)
            for i = 1:this.dSize
                ...
                pts = detectSURFFeatures(Im);
                [this.Dbase_desc{i},this.Dbase_kpt{i}] = extractFeatures(Im,pts,'Method','SURF');
            end
        end       
        function this = saveDbaseTemplates(this, saveName)
            for i = 1:this.dSize
                ...
                pts = detectSURFFeatures(Im);
                [desc{i},kpt{i}] = extractFeatures(Im,pts,'Method','SURF');
            end
        ...
        function D_t = findDifference(this,currImageId,prev_match_candidates)
            if this.pyrlevel == 1
                ...
                if this.method == 1
                    ...
                    for i = 1:length(prev_match_candidates)
                        [~, dist] = matchFeatures(this.Query_desc{currImageId},...
                            this.Dbase_desc{prev_match_candidates(i)},'MatchThreshold', 100, 'MaxRatio',1);
                        D(i) = sum(dist);
                    end
                end
            end
        ...
        function [match_candidates,quality] = findMatches(this,currImageId,prev_match_candidates)
            ...
            pts = detectSURFFeatures(Im);
            [desc,~] = extractFeatures(Im,pts,'Method','SURF'); 
