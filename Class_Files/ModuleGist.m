classdef ModuleGist < Config
    ...
    methods
        function this = init(this, HPC, Win)
            if HPC == 1
                addpath(genpath('/home/n7542704/MATLAB_2019_Working/Multi_SLAM_Fusion_Ver5/gist'));
            else
                if Win == 1
                    addpath(genpath('F:\MATLAB_2019_Working\Multi_SLAM_Fusion_Ver5\gist'));
                else
                    addpath(genpath('/media/HPC/MATLAB_2019_Working/Multi_SLAM_Fusion_Ver5/gist'));
                end
            end
        end    
        function this = setNumCandidates(this,NumCans,pyrlevel)
            if ((NumCans > 0) && (NumCans <= this.dSize))
                ...
                error('Please enter valid Gist module candidate count, between 1 and number of database images');
            end
